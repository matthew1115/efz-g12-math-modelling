  ...
  return (
    <>
      ...
      <SwitchPrimitive.Control
        class={cn(
          'inline-flex h-6 w-11 shrink-0 cursor-pointer items-center rounded-full border-2 border-transparent bg-input transition-[color,background-color,box-shadow] data-[disabled]:cursor-not-allowed data-[checked]:bg-foreground data-[disabled]:opacity-50',
          local.class,
        )}
        {...others}
      >
...
const SwitchThumb = <T extends ValidComponent = 'div'>(
  ...
  return (
    <SwitchPrimitive.Thumb
      class={cn(
        'pointer-events-none block size-5 translate-x-0 rounded-full bg-background/75 shadow-lg ring-0 transition-transform data-[checked]:translate-x-5',
        local.class,
      )}
      {...others}
    />
